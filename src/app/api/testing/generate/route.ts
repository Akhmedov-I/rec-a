...
/** Detect relevant regulatory standards based on position/description keywords. */
function detectStandardsContext(position: string, description: string): string {
  const text = `${position} ${description}`.toLowerCase();

  // IFRS/МСФО — must be checked BEFORE generic accounting
  if (/мсфо|ifrs|ias\b|международн.*стандарт.*финанс/.test(text)) {
    return 'Используй международные стандарты финансовой отчётности (МСФО/IFRS): IAS, IFRS, IASB. Вопросы должны касаться конкретных стандартов (IAS 1, IAS 2, IFRS 15, IFRS 16 и т.д.), раскрытий и учётной политики по МСФО.';
  }

  // Construction / Architecture / Engineering
  if (/строит|архитект|инженер|проектир|сметчик|прораб|геодез|электромонт|механик/.test(text)) {
    return 'Используй действующие нормативные документы Узбекистана: СНиП (Строительные нормы и правила) РУз, ТКП (Технические нормативы), ГОСТы, действующие в РУз, Технические регламенты Узбекистана, а также КМК (Қурилиш меъёрлари ва қоидалари). Вопросы должны касаться конкретных норм (нагрузки, пожарная безопасность, сейсмостойкость по нормам Узбекистана и т.д.).';
  }

  // Accounting / Finance (non-IFRS)
  if (/бухгалтер|финансист|аудитор|налог|учёт|учет|казначей/.test(text)) {
    return 'Используй нормативную базу Узбекистана: Налоговый кодекс РУз (НК РУз), Национальные стандарты бухгалтерского учёта (НСБУ), Закон РУз «О бухгалтерском учёте», инструкции Министерства финансов РУз. Вопросы должны проверять знание конкретных статей и норм.';
  }

  // HR / People / Recruiter
  if (/hr\b|кадр|рекрут|персонал|труд(?:ов)|зарплат|оклад/.test(text)) {
    return 'Используй Трудовой кодекс Республики Узбекистан (ТК РУз), нормативные акты Министерства занятости и трудовых отношений РУз, законодательство об охране труда в РУз. Вопросы проверяют знание конкретных норм ТК РУз (увольнение, отпуска, договоры и т.д.).';
  }

  // Legal / Law / Compliance
  if (/юрист|юридич|право(?:вед)?|адвокат|нотари|комплаенс|compliance/.test(text)) {
    return 'Используй законодательство Республики Узбекистан: Гражданский кодекс РУз (ГК РУз), Уголовный кодекс РУз, Хозяйственный процессуальный кодекс, Закон РУз «О договорно-правовой базе» и иные ЗРУ. Вопросы проверяют знание конкретных норм.';
  }

  // Medical / Healthcare
  if (/врач|медик|фармацевт|клиник|хирург|терапевт|стоматолог|медсестр/.test(text)) {
    return 'Используй клинические протоколы Министерства здравоохранения РУз, санитарные нормы и правила (СанПиН) РУз, приказы МЗ РУз. Вопросы охватывают диагностику, лечение и нормативную базу здравоохранения Узбекистана.';
  }

  // IT / Software / Dev
  if (/it\b|разработчик|developer|программист|devops|сисадмин|кибербезопас|data\s*engineer|аналитик.*данных/.test(text)) {
    return 'Используй международные стандарты: ISO/IEC 27001 (ИБ), OWASP, SOLID, REST/GraphQL, Agile/Scrum, CI/CD best practices. Вопросы проверяют практические знания соответствующих технологий, архитектурных паттернов и стандартов безопасности.';
  }

  // Logistics / Supply Chain
  if (/логист|снабжен|склад|закупк|procurement|supply/.test(text)) {
    return 'Используй законодательство РУз о госзакупках (Закон РУз «О государственных закупках»), INCOTERMS 2020, Таможенный кодекс РУз, нормативы складского хранения. Вопросы охватывают практику закупок и логистики в Узбекистане.';
  }

  // Default — generic international best practices
  return 'Используй общепринятые международные стандарты и лучшие практики в данной профессиональной области. Вопросы должны отражать реальные кейсы и актуальные требования рынка труда.';
}

function buildPrompt(name: string, position: string, description: string): string {
  const standardsCtx = detectStandardsContext(position, description);
